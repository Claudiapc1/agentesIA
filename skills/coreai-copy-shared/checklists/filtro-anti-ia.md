...
changelog:
  ...
  v3.3: |
    ...
    O filtro continua UNIVERSAL — qualquer cliente. Voice profile específico (Torriani, Halbert, etc) é etapa SEPARADA depois do filtro, não dentro dele.
  v3.2: |
    ...
    - §20 VALIDADOR REGEX EXECUTÁVEL (squad/scripts/anti-ia-validate.mjs)
    - §21 SUITE DE EXEMPLOS CANÔNICOS (squad/checklists/examples/)
...
- **Copy intercambiável** — troque "Torriani" por concorrente. Se nada vira mentira, a copy é genérica.
...
**Referência canônica:** Torriani — IA hobby vs IA ativo.
...
**Localização:** `[integração externa não empacotada]`
...
**Uso:**
```bash
../conteudo/scripts/validate-anti-ia.sh caminho/texto.md
../conteudo/scripts/validate-anti-ia.sh slides.html --html
../conteudo/scripts/validate-anti-ia.sh post.md --strict   # falha no 1º erro
```
...
**Uso:**
```bash
node [integração externa não empacotada] caminho/texto.md
echo "Seu texto" | node [integração externa não empacotada]
node [integração externa não empacotada] --json caminho/texto.md
```
...
**Uso:**
```bash
node [integração externa não empacotada] \
...
```
[integração externa não empacotada]
```
...
yq '.[] | select(.layer_1_llm.overall_verdict == "PASS") | .layer_1_llm.dimensions' outputs/antiia/audit-log.yaml
```
...
O audit-log pode conter copy de clientes. Ele vive em `outputs/antiia/audit-log.yaml`, nunca dentro do squad.
...
|--------------------|---------|------------|
| **Conceito proprietário não popularizado** | "Modelo Ferrari vs Toyota", "Filtro Anti-IA do Torriani", "Estratégia HTB", "Operação Imperial" | REPROVADO. Reescrever com vocabulário universal OU substituir por referência que o mercado conhece. |
