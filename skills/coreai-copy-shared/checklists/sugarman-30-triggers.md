...
## Thresholds vigentes no squad (verdicts executáveis)

Estes são os cortes que os workflows e o Oráculo já aplicam. Use-os, não invente número novo.

| Verdict | Corte | Consequência | Onde o squad já usa |
|---------|-------|--------------|---------------------|
| **FAIL** | < 15/30 | REPROVA. Não publica. Volta para reescrita | `checklists/oraculo-torriani.md:141`, `workflows/otimizacao-funil.md`, `workflows/funil-sala-secreta.md`, `workflows/wf-1-full-launch.yaml` |
| **REVISE** | 15-19/30 | Passa no piso mínimo mas não no gate de QA. Adicionar 3-5 triggers de alta prioridade e reavaliar | faixa entre o piso 15 e o gate 20 |
| **PASS** | >= 20/30 | Aprovado no QA Gate. Segue para delivery | `workflows/wf-copy-production.yaml:90`, `workflows/lancamento-completo.md:342` |
| **PASS+** | >= 25/30 | Aprovado com destaque | scoring guide acima |

**Regra de aplicação:**

- **Piso absoluto = 15.** Abaixo disso é reprovação instantânea em qualquer peça, em qualquer workflow. Nenhuma exceção.
- **Gate de QA = 20.** Peças que passam pelo `qa_gate` de um workflow formal (wf-copy-production, lancamento-completo) precisam de 20+ para PASS. Entre 15 e 19 o verdict é REVISE e a peça retorna à fase de criação (máx. 3 iterações).
- **Trigger de alta prioridade ausente conta em dobro na priorização** (não no score): se faltar HONESTY (1), STORYTELLING (6), BELONGING (21) ou CURIOSITY (23), esses são os primeiros a adicionar antes de qualquer outro.

**Contagem:** cada trigger vale 1 ponto, presente ou ausente. Sem pontuação parcial. Trigger "presente" significa que existe um trecho citável da copy que o executa, não que o tema foi tangenciado.
...
VERDICT (usar os cortes da secao "Thresholds vigentes no squad"):
□ 25-30: PASS+      - EXCEPTIONAL, ready to publish
□ 20-24: PASS       - STRONG, aprovado no QA Gate
□ 15-19: REVISE     - acima do piso, abaixo do gate. Add 3-5 triggers e reavalia
□ 10-14: FAIL       - WEAK, abaixo do piso 15. REPROVA
□ Below 10: FAIL    - NEEDS MAJOR REVISION. REPROVA

PISO ABSOLUTO: 15  (< 15 = reprova em qualquer workflow)
GATE DE QA:    20  (< 20 = REVISE em wf-copy-production / lancamento-completo)
```
...
- **Related tool**: agents/tier-2-executores/joe-sugarman.md (full 30 triggers reference)
