...
executor:
  ...
  rationale: "Stefan Georgi como criador do RMBC Method (Research, Mechanism, Brief, Copy) que gerou $700M+ em vendas"
```
...
```yaml
pre_conditions:
  - Produto/servico definido com informacoes de acesso
  - Avatar do cliente definido (ou tempo alocado para pesquisar na fase R)
  - Formato de output definido (sales page, VSL, email, ad)
  - Tempo disponivel para processo completo (6-10 horas estimadas)
  - Diagnostico de awareness e sofisticacao disponivel (recomendado)
```
...
rmbc_output:
  ...
  R_research:
    avatar: "Infoprodutor iniciante, 25-40 anos, quer viver de curso online"
    big_desire: "Liberdade financeira sem depender de emprego"
    main_objection: "Nao tenho audiencia suficiente"
    competitor_gap: "Ninguem ensina a validar antes de criar"

  M_mechanism:
    name: "Metodo Funil Invertido"
    statement: |
      "O Metodo Funil Invertido elimina meses de criacao de conteudo
      sem retorno ao inverter a ordem: valida a oferta PRIMEIRO,
      depois cria conteudo que vende."

  B_brief:
    lead_type: "Problem-Solution Lead"
    big_idea: "Voce nao precisa de audiencia. Precisa de uma oferta validada."
    proof_stack: ["3 cases de alunos", "Dados de 200 perfis", "Garantia 30 dias"]
    emotional_arc: "Frustracao → Descoberta → Esperanca → Acao"

  C_copy_excerpt: |
    "Voce ja postou 300 conteudos e ainda nao vendeu R$ 1?
    Nao e sua culpa. E a ORDEM que esta errada.

    A maioria dos 'gurus' ensina: crie conteudo → construa audiencia → venda.
    O problema? Voce gasta 6 meses criando conteudo pra descobrir que
    ninguem quer comprar o que voce vende.

    O Metodo Funil Invertido faz o contrario..."
```

## Veto Conditions
- Pular fase de Research (nunca escrever sem pesquisa)
- Brief incompleto usado como base para copy
- Mecanismo nao diferenciador do mercado
