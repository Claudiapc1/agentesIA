> DUPLICATA SEMÂNTICA: canônico é `tasks/otimizacao/sugarman-check.md` (comando `*sugarman-check`, usado no step 4d do tier_workflow e nas heuristics do copy-chief.md). Este arquivo cobre o mesmo domínio em modo agent-puro — consultar o canônico primeiro.
...
- **Related Agent**: agents/tier-2-executores/joe-sugarman.md
...
## Pre-Conditions
- Copy draft existente e completo (headline, body, CTA, oferta)
- Produto/servico e publico-alvo definidos
- Tipo de copy identificado (sales page, email, ad, landing page, VSL, webinar)
- Hopkins Audit ja executado (fundamentos cientificos primeiro, triggers depois)

## Output Example

```yaml
sugarman_enhancement:
  copy_type: "Sales page — Mentoria Elite"
  date: "2026-03-15"

  triggers_antes: 14/30
  triggers_depois: 21/30

  triggers_adicionados:
    - trigger: "Exclusividade (#7)"
      onde: "Abertura da oferta"
      antes: "Vagas abertas para a Mentoria Elite"
      depois: "Apenas 20 agencias serao aceitas nesta turma (processo seletivo obrigatorio)"

    - trigger: "Urgencia (#12)"
      onde: "Fechamento / CTA"
      antes: "Inscreva-se agora"
      depois: "As inscricoes fecham sexta as 23h59 — ou quando as 20 vagas acabarem (ja foram 14)"

    - trigger: "Honestidade (#1)"
      onde: "Secao de objecoes"
      antes: "Nosso programa funciona para todos"
      depois: "Vou ser honesto: se sua agencia fatura menos de 20k/mes, este programa NAO e pra voce. Voce precisa de clientes primeiro, nao de processo."

    - trigger: "Storytelling (#22)"
      onde: "Lead da pagina"
      adicionado: "Historia de como quase fechei minha agencia por burnout (3 paragrafos)"

  triggers_nao_adicionados:
    - "Greed (#4) — ja esta forte, adicionar mais soaria ganancioso"
    - "Guilt (#15) — nao se aplica ao contexto B2B"

  recomendacao: "21 triggers e o ponto ideal para high-ticket. Nao forcar mais."
```

## Veto Conditions
- Forcar todos os 30 triggers (exagero gera desconfianca)
- Aplicar triggers sem contexto do produto/mercado
- Ignorar triggers essenciais obrigatorios
