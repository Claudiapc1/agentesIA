...
- **Follows**: audit-copy.md (validate copy quality first)
...
## Pre-Conditions
- Copy validado e pronto para teste (passou por auditoria Hopkins)
- Elemento a testar definido (headline, offer, price, CTA, copy_length, image, format)
- Versao controle documentada
- Hipotese clara com metrica primaria de sucesso
- Trafego suficiente para significancia estatistica

## Output Example

```yaml
split_test:
  test_name: "Headline A vs B — Sales Page Mentoria Elite"
  date: "2026-03-15"
  status: "READY TO LAUNCH"

  hypothesis: |
    Headline com numero especifico (47 agencias) converte melhor que
    headline com promessa generica porque aumenta credibilidade
    imediata em mercado sofisticado (Stage 4).

  control_a:
    element: "Headline"
    copy: "Saia do operacional e escale sua agencia"
    current_ctr: "2.1%"

  variant_b:
    element: "Headline"
    copy: "47 donos de agencia fizeram isso para sair do operacional. Nenhum voltou."
    expected_lift: "+25-40%"

  test_config:
    primary_metric: "CTR do CTA principal"
    secondary_metrics: ["Time on page", "Scroll depth", "Form start rate"]
    traffic_split: "50/50"
    minimum_sample: 1000 visitantes por variante
    duration: "14 dias (ou ate significancia 95%)"
    significance_level: 0.95

  tracking:
    utm_control: "?utm_source=test&utm_content=headline-control"
    utm_variant: "?utm_source=test&utm_content=headline-variant"
    tool: "Google Optimize"

  decision_rules:
    if_b_wins: "Implementar B como novo controle, testar subheadline"
    if_a_wins: "Testar nova variante com angulo de dor (medo de burnout)"
    if_inconclusive: "Aumentar trafego ou testar elemento de maior impacto (CTA)"
```

## Veto Conditions
- Teste sem hipotese clara definida
- Variaveis demais testadas simultaneamente
- Teste sem metricas de sucesso pre-definidas
