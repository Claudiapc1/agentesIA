...
task:
  ...
  dependencies:
    - outputs/minds/{slug}/
    - data/expert-clone-mapping.yaml
```
...
executor:
  primary: copy-chief
  secondary: null
  rationale: "Copy Chief gerencia integracao de voice clones do MMOS com o sistema de producao de copy"
```
...
```yaml
pre_conditions:
  - Clone cognitivo existente em outputs/minds/{slug}/
  - Mapeamento expert-clone disponivel (data/expert-clone-mapping.yaml)
  - Expert identificado no briefing ou selecionado manualmente
  - Sistema MMOS operacional (fallback disponivel se offline)
```
...
voice_loaded:
  expert: "Jose Amorim"
  slug: "jose-amorim"
  source: "outputs/minds/jose_amorim/"
  status: "LOADED"

  voice_profile:
    tom: "Professoral, calmo, seguro"
    estilo: "Explicativo com analogias do cotidiano"
    vocabulario_core: ["cohort", "comunidade", "recorrencia", "engajamento"]
    frases_assinatura:
      - "Curso nao escala. Comunidade escala."
      - "Voce nao vende conhecimento, vende transformacao em grupo."

  calibracao:
    material_original_comparado: 15 # arquivos
    fidelidade_estimada: "87%"
    gaps_identificados:
      - "Humor sutil presente no original, sub-representado no clone"
...
## Veto Conditions
- Voice profile sem calibracao contra material original
- Loading sem fallback se MMOS nao disponivel
- Voice generico que nao reflete o copywriter
