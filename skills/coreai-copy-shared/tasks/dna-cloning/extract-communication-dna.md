...
dependencies:
  ...
  reference:
    - agents/tier-2-executores/david-ogilvy.md (communication_dna section)
    - agents/tier-1-estrategistas/dan-kennedy.md (communication_dna section)
  research:
...
- `agents/tier-2-executores/david-ogilvy.md` (communication_dna section)
- `agents/tier-1-estrategistas/dan-kennedy.md` (communication_dna section)
- `agents/tier-2-executores/gary-halbert.md` (communication_dna section)
...
executor:
  primary: copy-chief
  secondary: null
  rationale: "Copy Chief executa extracao de DNA comunicacional como Phase 3 do pipeline de criacao de agente copywriter"
```
...
pre_conditions:
  ...
  - Nome e slug do copywriter definidos
  - Template communication-dna-tmpl.yaml disponivel
  - Fase 2 (extract-frameworks) concluida (recomendado para contexto)
```
...
communication_dna:
  ...
  voice_architecture:
    tone_primary: "Provocador e direto"
    tone_secondary: "Sarcastico com humor seco"
    sentence_length_avg: "12-18 palavras"
    paragraph_style: "Curto, 2-4 frases maximo"
    person: "Primeira pessoa, voz de autoridade"

  vocabulary_tiers:
    tier_1_core: ["renegado", "sistema", "preguicoso", "magnetico", "cliente ideal"]
    tier_2_frequent: ["rebanho", "manada", "commodity", "premium"]
    tier_3_signature: ["marketing pecado mortal", "dinheiro na mesa"]

  rhetorical_devices:
    dominant: "Contraste direto (maioria vs poucos)"
    secondary: "Metaforas de guerra e caca"
    pattern: "Afirmacao provocadora → Evidencia → Consequencia"

  anti_patterns:
    never_says: ["talvez", "na minha humilde opiniao", "todo mundo pode"]
    never_does: "Pedir desculpa por ser caro ou exclusivo"

  disc_mapping: "D dominante (Dominancia), I secundario"
```

## Veto Conditions
- Extracao sem material fonte suficiente (minimo 3 fontes)
- DNA generico sem patterns especificos identificados
- Ignorar anti-patterns do copywriter original
