...
## PREMISSA OBRIGATÓRIA (Carregar ANTES de pesquisar)

> **CRITICAL:** Antes de iniciar qualquer pesquisa de avatar, carregue `data/premissa-core.md` e aplique o filtro "Problema Caro Pra Manter".

### Filtro de Público Premium

```yaml
ANTES_de_pesquisar_valide:
  - "[ ] O público-alvo JÁ FATURA (não é iniciante)"
  - "[ ] O problema deles CUSTA DINHEIRO real por mês"
  - "[ ] Eles já TENTARAM resolver antes"
  - "[ ] Eles querem VELOCIDADE, não aprender do zero"
  - "[ ] Eles querem quem RESOLVE, não quem ensina"

SE_algum_item_for_NAO:
  acao: "PARE e redefina o público. Estamos mirando no perfil errado."
  motivo: "Copy para público errado = dinheiro jogado fora"
```

### Quem NÃO É o Avatar

- Iniciantes querendo a primeira venda
- Curiosos que "um dia vão investir"
- Pessoas sem faturamento
- Quem quer "conteúdo de valor" gratuito
- Quem fala em "custo-benefício"

> **REGRA:** Se na pesquisa aparecerem sinais de "parasita de tempo" (ver premissa-core.md), DESCARTE esse perfil e foque no perfil premium.
...
executor:
  primary: copy-chief
  secondary: null
  rationale: "Copy Chief conduz diagnostico profundo de avatar integrando pesquisa de mercado com analise psicografica"
```
...
pre_conditions:
  ...
  - Produto/servico claro (o que sera vendido para este avatar)
  - Fontes de dados reais disponiveis (reviews, comentarios, entrevistas, forums)
  - Brand DNA preenchido (recomendado para contexto de marca)
```
...
```yaml
diagnostico_avatar:
  nome_avatar: "Marina — A Mentora Sobrecarregada"
  perfil: "Mulher, 32-42 anos, mentora de negocios, fatura 20-50k/mes"

  ponto_a:
    dor_principal: "Atende 1-a-1 sem parar e nao consegue escalar"
    emocoes: ["Exaustao", "Frustacao", "Sindrome da impostora"]
    crenca_limitante: "Se eu nao atender pessoalmente, o resultado cai"
    linguagem_exata:
      - "Estou no limite fisico e mental"
      - "Ganho bem mas nao tenho vida"
      - "Tenho medo de montar grupo e perder qualidade"

  ponto_b:
    resultado_desejado: "Faturar 100k/mes com grupo, trabalhando 4h/dia"
    emocoes: ["Liberdade", "Orgulho", "Tranquilidade"]
    mudanca_concreta: "Sair de 15 mentorados 1-a-1 para grupo de 50"

  objecoes_principais:
    - "Grupo nao funciona pra meu nicho" → Quebra: case da Julia que triplicou faturamento com grupo
    - "Nao sei criar metodologia" → Quebra: template de 7 pilares pronto
    - "Meus clientes querem exclusividade" → Quebra: modelo hibrido (grupo + hot seat)

  nivel_consciencia: "Solution-Aware (Level 3)"
  motivo_primario: "GAIN (riqueza + liberdade)"
```

## Veto Conditions
- Avatar baseado em suposicoes sem pesquisa real
- Ignorar dores emocionais (focar so em dores racionais)
- Avatar generico sem detalhes demograficos e psicograficos
