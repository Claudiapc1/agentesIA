> DUPLICATA SEMÂNTICA: canônico é `tasks/diagnostico/diagnose-awareness.md` (é o listado em `dependencies.tasks` no copy-chief.md). Este arquivo adiciona filtro de público premium ("Problema Caro Pra Manter") — útil como variante, mas consultar o canônico primeiro para o fluxo padrão.
...
## PREMISSA OBRIGATÓRIA: Diagnosticar Awareness do Público PREMIUM

> **CRITICAL:** O diagnóstico de awareness deve ser feito considerando o público "Problema Caro Pra Manter" (ver `data/premissa-core.md`), NÃO o mercado genérico.

```yaml
filtro_awareness:
  diagnosticar_para: "Público que já fatura e tem problema caro pra manter"
  NAO_diagnosticar_para: "Mercado genérico incluindo iniciantes e curiosos"

  implicacao: |
    O nível de awareness do público PREMIUM geralmente é Level 2-4.
    Raramente Level 1 (já sabem que têm problema, ele CUSTA dinheiro).
    Se o diagnóstico der Level 1, questione: estamos mirando no público errado?
```
...
sophistication_check: |
  After awareness level is set, also run diagnose-sophistication.md
...
- **Next Step**: diagnose-sophistication.md
...
executor:
  ...
  rationale: "Eugene Schwartz como autor do framework de 5 niveis de awareness (Breakthrough Advertising, 1966). Copy Chief valida o diagnostico."
```
...
```yaml
pre_conditions:
  - Produto/servico definido (o que esta sendo vendido)
  - Mercado-alvo identificado (quem e o publico)
  - Contexto de mercado disponivel (nicho, industria, cenario competitivo)
  - Pelo menos 1 sinal de mercado real (comportamento de busca, conversas de venda, feedback de clientes)
  - Premissa Core carregada (publico "Problema Caro Pra Manter")
```
...
```yaml
diagnosis:
  product: "Programa Acelerador Digital"
  market: "Infoprodutores iniciantes que querem viver de curso online"
  awareness_level: 2
  level_name: "Problem-Aware"
  confidence: MEDIUM

evidence:
  - "Buscam 'como ganhar dinheiro na internet' (problema, nao solucao)"
  - "Sabem que precisam de renda extra mas nao conhecem infoprodutos"
  - "Nao comparam plataformas porque nem sabem que existem"

copy_implications:
  headline_approach: "Agitacao do problema — nomear a dor e mostrar que solucao existe"
  copy_length: "Long (2000-5000 palavras)"
  proof_focus: "Provar que a solucao e possivel e acessivel"
  opening_strategy: "Empatia com a situacao atual antes de introduzir o conceito"

headline_templates:
  - "Se voce esta cansado de trocar tempo por dinheiro, leia isto"
  - "A frustracao de trabalhar 12h/dia e nao sobrar nada no final do mes"
  - "Finalmente existe um caminho para sair do CLT sem arriscar tudo"

next_step: |
  Diagnosticar sofisticacao do mercado para ajustar o nivel de prova necessario.
```
