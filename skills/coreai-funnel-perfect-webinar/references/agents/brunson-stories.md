# brunson-stories — contexto de escrita

Conteúdo metodológico selecionado. Exemplos são ilustrativos, nunca provas do cliente. O contrato do SKILL.md prevalece; não ativar persona externa nem executar handoffs.
...
heuristics:
  - id: H_001
    when: "Usuario pede historia sem especificar tipo ou proposito"
    then: "Diagnosticar intencao: vender (Epiphany Bridge), posicionar (Origin Story), quebrar crenca (Belief Story)"
    why: "Cada tipo de historia tem estrutura diferente. Historia sem proposito e anedota."
  - id: H_002
    when: "Usuario menciona Epiphany Bridge ou historia de epifania"
    then: "Aplicar estrutura completa de 8 passos: Backstory → Desire → Wall → Epiphany → Plan → Conflict → Achievement → Transformation"
    why: "Epiphany Bridge sem os 8 passos perde impacto. Cada passo cria tensao e resolucao."
  - id: H_003
    when: "Usuario menciona origin story, historia de origem"
    then: "Construir com foco em vulnerabilidade + ponto de virada + transformacao. Mostrar o humano, nao o guru."
    why: "Origin story que so mostra sucesso afasta. Vulnerabilidade cria conexao."
  - id: H_004
    when: "Usuario precisa de historias para webinar (3 Secrets)"
    then: "Criar 3 historias de falsa crenca: Vehicle Story, Internal Story, External Story — cada uma quebrando um tipo"
    why: "3 Secrets do Perfect Webinar exigem 3 historias especificas. Cada uma derruba uma categoria de objecao."
  - id: H_005
    when: "Usuario menciona story bank, inventario de historias"
    then: "Criar inventario com 10-15 historias categorizada por tipo (venda, autoridade, conexao, prova) com uso sugerido"
    why: "Expert com inventario de historias nunca fica sem conteudo. Cada historia tem multiplos usos."
  - id: H_006
    when: "Historia entregue nao conecta emocionalmente"
    then: "Verificar se tem Kinda Like Bridge — analogia que conecta experiencia do narrador ao mundo do ouvinte"
    why: "Sem Kinda Like Bridge, historia fica distante. Ponte de analogia cria identificacao imediata."
  - id: H_007
    when: "Usuario menciona Hero's Two Journeys"
    then: "Estruturar jornada de conquista (externa) + jornada de transformacao (interna) em paralelo"
    why: "Audiencia lembra da transformacao interna, nao da conquista externa. Duas jornadas criam profundidade."
