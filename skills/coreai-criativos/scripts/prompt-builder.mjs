 ...
 */
function buildCtaButtonRule(cta, accent) {
...
const EXPERT_ADJUSTMENT_INSTRUCTIONS = {
...
function buildExpertAdjustmentsBlock(input) {
    if (!input)
        return null;
    ...
    for (const line of presetLines)
        lines.push(`- ${line}`);
    if (notes)
        lines.push(`- Outras particularidades: ${notes}`);
    lines.push("Se houver conflito entre o que o template mostra e estes ajustes, OS AJUSTES ACIMA VENCEM. O template define pose, enquadramento, cenário e layout — não define acessórios, roupa ou expressão do expert.");
    return lines.join("\n");
}
/**
...
export function buildPrompt(project) {
    const { copy, brand, format, hasExpertPhotos, hasLogo, variationIndex, forceVariation, varyClothing, hasCustomBackground, backgroundMode, blockColor, templateMiniPrompt, templateAnalysis, templateBackground, templateTextLayout, templatePerson, templateSpacing, templateLogoSizePct, chatRefinement, imageRefs, expertAdjustments, activeCopyFields, } = project;
    ...
    const isCopyFieldActive = (field) => {
        if (!activeCopyFields || activeCopyFields.length === 0)
            return true;
...
- NUNCA use a autoridade de conteúdo como desculpa pra redesenhar a forma: trocar cor/texto/rosto NÃO muda posição, tamanho ou fonte de nada.`);
    ...
    if (templateTextLayout?.length) {
        const textDesc = templateTextLayout.map((item) => {
            ...
            if (item.position)
                parts.push(`posição ${item.position}`);
            if (item.grid_area)
                parts.push(`no ${item.grid_area}`);
            if (item.size_pct)
                parts.push(`ocupando ~${item.size_pct}% da altura`);
            if (item.style)
                parts.push(`estilo: ${item.style}`);
            if (item.color)
                parts.push(`cor: ${item.color}`);
            if (item.lines)
                parts.push(`${item.lines} linha${item.lines > 1 ? "s" : ""}`);
            return parts.join(", ");
        }).join("\n  ");
    ...
    if (templatePerson?.present) {
        const personParts = [];
        if (templatePerson.framing)
            personParts.push(`enquadramento: ${templatePerson.framing}`);
        if (templatePerson.grid_position)
            personParts.push(`posição: ${templatePerson.grid_position}`);
        if (templatePerson.coverage_pct)
            personParts.push(`ocupa ~${templatePerson.coverage_pct}% do quadro`);
        if (templatePerson.pose)
            personParts.push(`pose: ${templatePerson.pose}`);
        if (templatePerson.clothing)
            personParts.push(`vestimenta: ${templatePerson.clothing}`);
        if (templatePerson.expression)
            personParts.push(`expressão: ${templatePerson.expression}`);
        if (templatePerson.gaze_direction)
            personParts.push(`olhar: ${templatePerson.gaze_direction}`);
    ...
    if (templateSpacing) {
        const spacingParts = [];
        if (templateSpacing.overall_density)
            spacingParts.push(`densidade: ${templateSpacing.overall_density}`);
        if (templateSpacing.text_blocks_gap)
            spacingParts.push(`gap entre textos: ${templateSpacing.text_blocks_gap}`);
        if (templateSpacing.margin_edges_pct)
            spacingParts.push(`margem das bordas: ~${templateSpacing.margin_edges_pct}%`);
...
Ajuste a micro-expressão facial e a energia emocional da cena para reforçar essa mensagem — mantendo ABSOLUTAMENTE a composição e o layout do template${wantsVariation ? " (a pose/cenário/fundo variam conforme a VARIAÇÃO OBRIGATÓRIA do topo)" : ", enquadramento, pose e cenário do template"}. ${wantsVariation ? "" : "Apenas a energia sutil e a expressão devem conversar com a copy."}`);
    ...
    const copyParts = [];
    if (copy.headline && isCopyFieldActive("headline"))
        copyParts.push(`Headline (texto principal, destaque): ${copy.headline}`);
    if ((copy.subheadline || copy.mini_copy) && isCopyFieldActive("subheadline"))
        copyParts.push(`Subheadline (apoio): ${copy.subheadline || copy.mini_copy}`);
    if ((copy.ponte || copy.list_items) && isCopyFieldActive("ponte"))
        copyParts.push(`Ponte (corpo que conecta): ${copy.ponte || copy.list_items}`);
...
É PROIBIDO inverter, intercalar ou espalhar esses blocos em áreas diferentes da arte. Todos vivem DENTRO da área de conteúdo que o template define (o painel/box/área de texto do template) — nenhum texto da copy pode ir parar fora dela (ex: no topo da imagem, sobre a foto, no lugar do nome de pessoa do template).`;
    ...
    const looksLikePersonName = (t) => {
        ...
        if (words.length < 2 || words.length > 7)
            return false;
        if (s === s.toUpperCase())
            return false; // ALL CAPS = título, não nome
    ...
    if (templateTextLayout?.length) {
        ...
        templateTextLayout.forEach((item, idx) => {
            ...
            if (copyField && copy[copyField] && isCopyFieldActive(copyField)) {
                if (!byField.has(copyField))
                    byField.set(copyField, []);
        ...
        for (const [copyField, idxs] of byField) {
            ...
            const best = [...idxs].sort((a, b) => {
                const ia = templateTextLayout[a];
                const ib = templateTextLayout[b];
                const score = (it) => (it.size_pct ?? 1) - (looksLikePersonName(it.text_found) ? 1000 : 0);
            ...
            const propParts = [];
            if (item.size_pct)
                propParts.push(`~${item.size_pct}% da altura`);
            if (item.position)
                propParts.push(`posição: ${item.position}`);
            if (item.style)
                propParts.push(`estilo: ${item.style}`);
...
IMPORTANTE: isto NÃO proíbe o BOTÃO DE CTA da marca${copy.cta && isCopyFieldActive("cta") ? ` (com o texto "${copy.cta}")` : ""}. O botão de CTA descrito acima É parte do design do criativo e DEVE aparecer. O que se proíbe é imitar a UI nativa da Meta (ícones de rede social, swipe, barra de stories), não o botão da campanha.`);
    ...
    if (hasExpertPhotos) {
        forbidden.push("- NÃO cole, recorte nem reproduza a foto de referência da pessoa na arte final — ela serve só como fonte de identidade facial", 
        ...
        wantsVariation
            ...
            : "- NÃO reproduza a pose, a roupa, o fundo ou a iluminação da foto de referência — use SEMPRE a pose e o cenário do template", "- NÃO deixe o resultado parecer uma colagem ou um sticker de foto sobre o template");
...
function mapRoleToCopyField(role) {
    const map = {
        headline: "headline",
...
export function buildImageEditPrompt(input) {
    const { originalPrompt, userInstruction, hasExpertPhotos, hasLogo = false, templateReferenceCount, expertPhotoMode = "preserve", expertAdjustments, brand, copy, contextHint, } = input;
    const lines = [
...
Toda cor visível na arte final (fundo, formas, textos, gradientes, overlays, badges, ícones decorativos) DEVE estar dentro desta paleta. Se o criativo atual usa cores fora desta paleta, AJUSTE-AS para a paleta da marca durante a edição. Toda fonte de texto DEVE ser da tipografia da marca.${hasLogo ? "\nO logo da marca é a imagem anexada — substitua qualquer outro logo/símbolo de empresa que apareça no criativo atual por este logo oficial." : ""}`);
    ...
    if (copy && (copy.headline || copy.subheadline || copy.ponte || copy.cta)) {
        const repl = [];
        if (copy.headline)
            repl.push(`• HEADLINE (texto principal/maior do criativo) → troque por: "${copy.headline}"`);
        if (copy.subheadline || copy.mini_copy)
            repl.push(`• SUBHEADLINE (texto de apoio, menor que a headline) → troque por: "${copy.subheadline || copy.mini_copy}"`);
        if (copy.ponte || copy.list_items)
            repl.push(`• PONTE (corpo que conecta a ideia ao CTA) → troque por: "${copy.ponte || copy.list_items}"`);
 ...
 */
export function buildTemplateAwareCopyPrompt(input) {
 ...
 */
export function buildBriefingImageReferenceRule(instruction) {
 ...
 */
export function buildPromptFromBriefingItem(input) {
...
${visualDirection}`);
    ...
    const textParts = [];
    if (headline)
        textParts.push(`- Headline (destaque principal, maior): "${headline}"`);
    if (subheadline)
        textParts.push(`- Subheadline (apoio, menor que a headline): "${subheadline}"`);
    if (ponte)
        textParts.push(`- Ponte (corpo que conecta a ideia ao CTA): "${ponte}"`);
